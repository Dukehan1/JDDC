...
from utils import init_embedding, device_cuda, gen_data, prepare_vocabulary, ComputeR10_1, get_minibatches, \
    sentence_with_kb, word_tokenizer, char_tokenizer
...
dev_id = 0
model_dir = 'smn-char-kb-model-' + str(dev_id)
...
def run_train():
    ...
    # 生成词表（word）
    if os.path.exists(model_dir + '/vocab_char'):
        t = open(model_dir + '/vocab_char', 'rb')
        vocab_char = pickle.load(t)
        t.close()
    else:
        vocab_char = prepare_vocabulary(data, cut=cut)
        t = open(model_dir + '/vocab_char', 'wb')
    ...
    if os.path.exists(model_dir + '/data'):
        ...
        train_examples = gen_data(vocab_char, data['trainQuestions'], data['trainAnswers'], 1, max_length, max_num_utterance)
        dev_examples = gen_data(vocab_char, data['devQuestions'], data['devAnswers'], 10, max_length, max_num_utterance)
    ...
    embed = init_embedding(embed_size, vocab_char.n_words, vocab_char.word2index, True)
    embedding = nn.Embedding(vocab_char.n_words, embed_size, padding_idx=0).from_pretrained(embed, freeze=False)
    ...
    trainIters(vocab_char, embedding, smn, optimizer, train_examples, dev_examples, n_epochs, lr, batch_size,
...
def prediction(input_data, candidate_answers):
    ...
    # smn
    embedding = nn.Embedding(vocab_char.n_words, embed_size, padding_idx=0)
    smn = SMN(embed_size, hidden_size, compression_hidden_size, max_num_utterance, max_length,
              bidirectional=bidirectional, num_layers=num_layers, dropout_p=dropout_p)
    ...
    for i in range(len(input_data)):
        ...
        examples = gen_data(vocab_char, candidate_utterances, candidate_response, 1, max_length, max_num_utterance)
